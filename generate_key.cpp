...
std::vector<pgp::packet> generate_key(const master_key &master, std::string user, uint32_t creation, uint32_t signature, uint32_t expiration, boost::string_view context)
{
    ...
    derived_key<secret_key_size>    encryption_key_derivation       { master, 2, context };
    derived_key<secret_key_size>    authentication_key_derivation   { master, 3, context };
    ...
    auto &encryption_key = mpark::get<pgp::secret_subkey>(packets[3].body());
    ...
    auto &authentication_key = mpark::get<pgp::secret_subkey>(packets[5].body());
